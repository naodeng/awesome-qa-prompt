...
- 🔄 **版本齐全**：每个模块提供完整版和精简版，部分模块新增 `all_round`、`Web`、`Mobile` 三种平台场景版
...
- **文件位置**：`testing-types/zh/functional-testing/Standard-version/FunctionalTestingPrompt.md`
- **功能描述**：设计功能测试策略和执行方案
- **适用场景**：功能测试执行、回归测试
- **核心能力**：
  - 黑盒测试方法和功能分解测试
...
- **文件位置**：`testing-types/zh/performance-testing/Standard-version/PerformanceTestingPrompt.md`
...
- **文件位置**：`testing-types/zh/automation-testing/Standard-version/AutomationTestingPrompt.md`
- **功能描述**：自动化测试脚本设计和框架选择
...
- **文件位置**：`testing-types/zh/mobile-testing/Standard-version/MobileTestingPrompt.md`
- **功能描述**：移动应用测试策略和场景设计
- **适用场景**：移动应用测试、兼容性测试
- **核心能力**：
...
- **文件位置**：`testing-types/zh/bug-reporting/Standard-version/BugReportingPrompt.md`
- **功能描述**：标准化缺陷报告模板和缺陷分析
- **适用场景**：缺陷管理、缺陷分析
- **核心能力**：
  - 缺陷识别原则和分类体系
...
- **文件位置**：`testing-types/zh/test-reporting/Standard-version/TestReportingPrompt.md`
- **功能描述**：测试执行报告和质量分析报告
- **适用场景**：测试总结、质量评估
- **核心能力**：
  - 多类型测试报告编写
...
- **文件位置**：`testing-types/zh/test-strategy/Standard-version/TestStrategyPrompt.md`
...
- **文件位置**：`testing-types/zh/ai-assisted-testing/Standard-version/AIAssistedTestingPrompt.md`
...
- **文件位置**：`testing-types/zh/manual-testing/Standard-version/ManualTestingPrompt.md`
- **功能描述**：手动测试策略和探索性测试
- **适用场景**：探索性测试、用户体验测试
- **核心能力**：
  - 探索性测试方法和技巧
...
- **文件位置**：`testing-types/zh/security-testing/Standard-version/SecurityTestingPrompt.md`
- **功能描述**：安全漏洞检测和安全测试策略
- **适用场景**：安全审计、渗透测试
- **核心能力**：
  - OWASP Top 10 漏洞测试
...
- **文件位置**：`testing-types/zh/api-testing/Standard-version/APITestingPrompt.md`
- **功能描述**：API接口测试和集成测试
- **适用场景**：接口测试、微服务测试
- **核心能力**：
  - RESTful API测试设计
...
- **文件位置**：`testing-types/zh/accessibility-testing/Standard-version/AccessibilityTestingPrompt.md`
...
- **文件位置**：`testing-types/zh/test-case-reviewer/Standard-version/TestCaseReviewerPrompt.md`
- **功能描述**：专业的测试用例审核和质量把控
...
部分测试类型还提供平台场景版本：
- **`all_round`**：同时覆盖 Web 端和移动端
- **`Web`**：聚焦浏览器和 Web 应用场景
- **`Mobile`**：聚焦 iOS / Android 应用场景
...
📖 了解更多：[提示词框架介绍](./prompt-frameworks/zh/README.md)
...
- **文件位置**：`Workflows/zh/sprint-testing-workflow.md` | [English](Workflows/en/sprint-testing-workflow.md)
...
- **文件位置**：`Workflows/zh/release-testing-workflow.md` | [English](Workflows/en/release-testing-workflow.md)
- **适用场景**：生产环境发布前的全面测试
- **涵盖内容**：
  - 发布规划和测试准备（T-14天）
  - 专项测试（性能、安全、可访问性）
...
访问在线文档网站：**[https://inaodeng.com/prompts](https://inaodeng.com/prompts)**

1. 在左侧导航栏选择测试类型或工作流程
2. 按需选择"完整版"、"精简版"或平台场景版（`all_round` / `Web` / `Mobile`）
...
1. **选择提示词**
   - 根据你的测试任务，选择对应的提示词文件
   - 中文项目使用 `zh/` 目录下提示词，英文项目使用 `en/` 目录下提示词
...
- 提示词文件内不再重复写“使用说明”提示文案
- 使用时请统一按以下方式操作：
  - 选择一个提示词文件
  - 从第一个分隔线 `---` 开始，复制下方全部内容到 AI 助手
  - 在提示词后补充你的项目背景、需求或测试资料
  - 让 AI 生成结果后，再结合实际情况进行人工校对

### 示例：需求分析

```markdown
# 步骤 1：选择提示词版本
# 标准版本：testing-types/zh/requirements-analysis/Standard-version/RequirementsAnalysisPrompt.md
# ROSES 框架版本：testing-types/zh/requirements-analysis/ROSES-version/RequirementsAnalysis-ROSES-Full.md
# LangGPT 框架版本：testing-types/zh/requirements-analysis/LangGPT-version/RequirementsAnalysis-LangGPT-Full.md
# 其他框架版本类似...
...
```markdown
# 完整的测试项目流程（标准版本示例）
1. 需求分析 → testing-types/zh/requirements-analysis/Standard-version/RequirementsAnalysisPrompt.md
2. 测试策略 → testing-types/zh/test-strategy/Standard-version/TestStrategyPrompt.md
3. 测试用例编写 → testing-types/zh/test-case-writing/Standard-version/TestCaseWritingPrompt.md
4. 功能测试 → testing-types/zh/functional-testing/Standard-version/FunctionalTestingPrompt.md
5. 自动化测试 → testing-types/zh/automation-testing/Standard-version/AutomationTestingPrompt.md
6. 性能测试 → testing-types/zh/performance-testing/Standard-version/PerformanceTestingPrompt.md
7. 移动端测试 → testing-types/zh/mobile-testing/Standard-version/MobileTestingPrompt.md
8. 缺陷上报 → testing-types/zh/bug-reporting/Standard-version/BugReportingPrompt.md
9. 测试报告 → testing-types/zh/test-reporting/Standard-version/TestReportingPrompt.md

# 也可以使用框架版本，例如 ROSES 框架：
1. 需求分析 → testing-types/zh/requirements-analysis/ROSES-version/RequirementsAnalysis-ROSES-Full.md
2. 测试策略 → testing-types/zh/test-strategy/ROSES-version/TestStrategy-ROSES-Full.md
# ... 其他步骤类似
```
...
│   ├── en/README.md                   # 英文工作流程说明
│   ├── zh/daily-testing-workflow.md   # 日常测试工作流程（中文）
│   ├── en/daily-testing-workflow.md   # 日常测试工作流程（英文）
...
1. **选择合适的版本**：
   ...
   - **语言选择**：中文项目使用 `zh/` 目录下提示词，英文项目使用 `en/` 目录下提示词
...
仓库级 Prompt 规范已整理到 [PROMPT_AUTHORING_STANDARD.md](./PROMPT_AUTHORING_STANDARD.md)。
...
- 新增测试类型 Prompt
- 修改模块 README 或默认入口
- 新增 CRISPE / RISE / ICIO / ROSES / LangGPT 变体
- 做批量巡检和一致性评审
...
3. **修改内容**
   - 在 `testing-types/` 或 `Workflows/` 中编辑对应文件
   - 需要双语同步时，同时更新 `zh/` 与 `en/` 内容
...
**更多赞助方式请访问**：[在线文档 - 赞助支持](https://inaodeng.com/prompts)
...
### [v0.5.0] - 2026-03-29 🧹 文档信息清理与目录优化

#### ✨ 主要更新
- **移除在线文档站相关信息**：
  - 从仓库说明中移除文档站本地开发与部署配置描述
  - 保留统一在线入口链接：[https://inaodeng.com/prompts](https://inaodeng.com/prompts)

- **优化项目目录结构**：
  - 测试类型目录统一调整为：`testing-types/<语言>/<模块>/<版本>/<文件>`
  - 工作流程目录统一为：`Workflows/<语言>/<文件>`
  - README 中目录示例与现有仓库结构保持一致

#### 🔧 体验改进
- 提示词文件内不再重复“使用说明”提示语
- 在 README 中提供统一的使用步骤，减少重复信息
