...
- Reconstruct the end-to-end flow by actor and state; mark decisions, system handoffs, data changes, failure paths, and recovery paths; derive test concerns from flow nodes.
...
- Specialized focus: for "business flow analysis", identify its own core targets, distinctive failure modes, decision rules, and evidence; do not substitute generic checks from the broader domain.
...
3. Result table: flow node, actor, pre-state, input or event, rule, resulting state, exception and recovery, test concern

| flow node | actor | pre-state | input or event | rule | resulting state | exception and recovery | test concern |
| --- | --- | --- | --- | --- | --- | --- | --- |
