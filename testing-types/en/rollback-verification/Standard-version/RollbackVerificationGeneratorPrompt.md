...
- Define pre-rollback checkpoints and data protection; cover applications, configuration, schema compatibility, in-flight transactions, and external dependencies; state success evidence, abort conditions, and roll-forward alternatives.
...
- Specialized focus: for "rollback verification", identify its own core targets, distinctive failure modes, decision rules, and evidence; do not substitute generic checks from the broader domain.
...
3. Result table: phase, object, precondition, rollback action, expected state, validation evidence, abort or roll-forward condition

| phase | object | precondition | rollback action | expected state | validation evidence | abort or roll-forward condition |
| --- | --- | --- | --- | --- | --- | --- |
