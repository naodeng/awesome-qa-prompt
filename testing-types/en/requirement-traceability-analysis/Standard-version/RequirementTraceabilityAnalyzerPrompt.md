...
- Build bidirectional mappings using stable identifiers; distinguish complete, partial, indirect, and missing coverage; verify links point to real evidence rather than names only; identify orphan items.
...
- Specialized focus: for "requirement traceability analysis", identify its own core targets, distinctive failure modes, decision rules, and evidence; do not substitute generic checks from the broader domain.
...
3. Result table: requirement or control, source, linked artifact, relationship type, coverage status, evidence, gap and action
