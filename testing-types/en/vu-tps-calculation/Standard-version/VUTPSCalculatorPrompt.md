...
- State the queueing or concurrency approximation used; normalize units and show calculations; provide assumptions, ranges, and sensitivity analysis instead of false precision.
...
- Specialized focus: for "vu tps calculation", identify its own core targets, distinctive failure modes, decision rules, and evidence; do not substitute generic checks from the broader domain.
...
3. Result table: input, source, unit, formula, calculation, estimate range, limitation

| input | source | unit | formula | calculation | estimate range | limitation |
| --- | --- | --- | --- | --- | --- | --- |
