...
- Reconstruct the minimal reproduction path and timeline; separate facts, symptoms, and causal hypotheses; compare healthy and failing samples; design a validation step for each candidate cause.
...
- Specialized focus: for "bug analysis", identify its own core targets, distinctive failure modes, decision rules, and evidence; do not substitute generic checks from the broader domain.
...
3. Result table: symptom, reproduction condition, evidence, impact, candidate cause, validation method, confidence

| symptom | reproduction condition | evidence | impact | candidate cause | validation method | confidence |
| --- | --- | --- | --- | --- | --- | --- |
