...
<!-- Prompt purpose: Supports Quality Metrics Analysis by organizing input evidence, constraints, risks, validation priorities, decision criteria, and actionable QA next steps without inventing facts. -->
...
- Validate that each metric is computable, interpretable, and decision-relevant; inspect denominators, sampling bias, lag, and incentive side effects; do not invent missing targets.
...
- Specialized focus: for "quality metrics analysis", identify its own core targets, distinctive failure modes, decision rules, and evidence; do not substitute generic checks from the broader domain.
...
3. Result table: metric, decision purpose, formula and definition, data source, baseline or target, interpretation, limitation and anti-misuse recommendation
