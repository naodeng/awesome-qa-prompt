...
- First verify data comparability; correlate client, service, database, and dependency metrics over time; use evidence to support or reject bottleneck hypotheses.
...
- Specialized focus: for "performance bottleneck analysis", identify its own core targets, distinctive failure modes, decision rules, and evidence; do not substitute generic checks from the broader domain.
...
3. Result table: symptom, load stage, metric change, correlated evidence, candidate cause, validation experiment, confidence

| symptom | load stage | metric change | correlated evidence | candidate cause | validation experiment | confidence |
| --- | --- | --- | --- | --- | --- | --- |
