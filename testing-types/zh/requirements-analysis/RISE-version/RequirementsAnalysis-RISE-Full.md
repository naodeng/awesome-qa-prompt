# 需求分析 - RISE框架 (完整版) - Default版

<!-- Prompt purpose: 用于需求分析 - RISE框架 (完的风险识别、证据梳理与可执行测试建议输出。 -->
## 平台场景说明
...
**Input 输入：** 基于提供的需求文档（包含业务目标、功能描述、用户角色、业务规则、技术架构、接口定义等信息），进行全面的需求理解和信息提取，为测试分析提供准确的输入基础

**Steps 步骤：** 按照系统化的步骤进行需求分析：1）需求理解与信息提取 2）业务背景分析 3）测试范围界定 4）测试场景设计 5）测试方法规划 6）测试策略建议 7）覆盖度分析 8）格式化输出

**Expectation 期望：** 输出结构化的需求分析报告，包含完整的业务背景分析、明确的测试范围界定、全面的测试场景设计（正向、异常、边界、安全、性能、兼容性）、科学的测试方法规划和实用的测试策略建议，为后续测试活动提供科学指导
