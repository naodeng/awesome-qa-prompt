...
<!-- Prompt purpose: 用于API测试 - RISE框架 (的风险识别、证据梳理与可执行测试建议输出。 -->
---
...
**Input 输入：** 基于API文档和测试需求（包含API文档、API类型、系统架构、测试目标、质量要求、工具选择等信息），进行快速分析和理解，为API测试策略制定提供准确的输入基础

**Steps 步骤：** 按照系统化的步骤进行API测试策略制定：1）API分析 2）策略设计 3）用例设计 4）自动化集成

**Expectation 期望：** 输出简洁的API测试方案文档，重点突出测试概述、API测试策略、功能测试设计、性能测试、安全测试、自动化框架等核心内容，为项目决策提供可执行的API测试策略建议
