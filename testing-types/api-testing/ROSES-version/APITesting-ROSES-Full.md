...
**Objective 目标：** 根据提供的API文档、系统架构或测试需求，设计全面的API测试策略和测试方案，确保API测试覆盖完整、方法科学、自动化程度高，并能有效保证API的功能正确性、性能表现和安全性

**Scenario 场景：** 需要为API或API驱动的系统设计测试方案，涉及API功能验证、性能测试、安全测试、契约测试、集成测试等方面，需要综合考虑API协议类型、系统架构、业务需求、质量要求等多重因素

**Expected Solution 预期解决方案：** 输出详细的API测试方案文档，包含测试概述、API文档分析、API测试策略、详细测试方案、专项测试方案、API测试自动化、API测试报告等完整内容，提供可执行的API测试用例和自动化脚本

**Steps 步骤：** API分析 → 策略制定 → 用例设计 → 脚本开发 → 自动化集成 → 持续优化
